/* app.js - gabungan UI improved + konfirmasi WA sesuai original (convert.html)
   Pastikan file ini berada di assets/js/app.js dan index.html memanggilnya dengan defer.
*/
...
(function(){
  ...
  const onchainAddrs = {
    evm: '0x35869dc158db5565f48d9bf80339e1f16cff4374',
    aptos: '0x3fc53d3ecbd3e88bb51ff51c0d449d28db779af6b67ee3ecddea25205b1627f3',
    solana: '8J5FDdydRn3pPfHrF6AAoP5hoxn2sNVD8QJPzgkXsU9i'
  };
  ...
  // audio error beep using WebAudio
  ...
  // update crypto view (automatic) — similar logic to original convert.html
  ...
  async function updateCryptoView(){
    ...
    if(ex === 'Wallet Crypto / Onchain'){
      ...
      if(!selectedChain){
        ...
        const label = (selectedChain === 'evm') ? 'EVM (BEP-20 / ETH / Polygon / dll)' : (selectedChain === 'aptos' ? 'APTOS' : 'SOLANA');
  ...
  // confirm via WA: logic restored from original convert.html
  confirmBtn.addEventListener('click', async ()=>{
    ...
    if(mode==='crypto'){
      ...
      summaryHtml += `Jenis: <strong>Crypto</strong><br/>Exchange: <strong>${ex}</strong><br/>Nominal: <strong>${usd} USD</strong><br/>Rate: <strong>Rp ${formatIDR(rate)}</strong><br/>Fee: <strong>Rp ${formatIDR(feeVal)}</strong><br/>Hasil akhir (est.): <strong>Rp ${formatIDR(final)}</strong><br/>`;
      if(addrText) summaryHtml += `Tujuan: <div class="addr-box"><div class="addr-text">${addrText}</div></div>`;
      summaryHtml += `<br/>`;
    ...
    // build WA message (note: original used manual %0A newlines)
    let body = `Halo admin,%0AInvoice: ${historyItem.invoice}%0A`;
    if(historyItem.mode === 'crypto'){
      body += `Jenis: Crypto%0AExchange: ${historyItem.exchange}%0ANominal: ${historyItem.nominal}%0AHasil: Rp ${formatIDR(historyItem.result)}%0AFee: Rp ${formatIDR(historyItem.fee)}%0A`;
