/* assets/js/app.js
   Versi: lock-preview-pulsa-ewallet
   Perubahan utama:
   - Pulsa & E-Wallet: jangan tampilkan preview saat mengetik. Tampil HANYA setelah tombol Konfirmasi ditekan.
   - Tombol Konfirmasi -> tampilkan hasil/tujuan & lock input. Batal -> unlock & sembunyikan hasil.
   - Crypto behavior tetap (hasil muncul saat nominal dipilih).
   - Menggunakan cached rate agar hasil crypto terasa instan.
*/

(function(){
  const BASE_RATE = 16500;
  const MAX_USD = 200;
  const waAdmin = '6281234567890'; // update sesuai nomor admin WA
  ...
  // populate USD select (no default)
  ...
  // -----------------------
  // Crypto compute & view (same fast behavior)
  // -----------------------
  ...
  function applyCryptoCalculation(usdVal, rate, ex, chain){
    ...
    if(ex === 'Wallet Crypto / Onchain'){
      ...
      `;
      infoArea.classList.remove('hidden');
      if(ex){ confirmWrap.classList.remove('hidden'); confirmWrap.classList.add('slide-up'); } else hide(confirmWrap);
    }
  }
  ...
  // -----------------------
  // Pulsa: IMPORTANT CHANGE
  // - Input/Operator change => HIDE preview (do not auto-show)
  // - Only confirmPulsa click shows preview & locks
  // -----------------------
  function computePulsaPreview(){
    ...
    // NOTE: confirmWrap (WA confirm) will be shown by caller after compute if desired
  }

  // On confirmPulsa: validate -> show preview -> lock -> show WA confirm
  confirmPulsa.addEventListener('click', ()=>{
    if(!pulsaLocked){
      ...
      // show global WA confirm button area
  ...
  // On typing: HIDE preview (do not compute/show)
  pulsaAmountEl.addEventListener('input', ()=> {
    if(pulsaLocked) return; // if locked, ignore typing (field disabled anyway)
    // hide preview until user clicks Konfirmasi
    hide(resultCard); hide(confirmWrap); hide(pulsaTargetWrap);
  });

  // operator change: hide preview unless already locked (then recompute preview with locked value)
  pulsaOperator.addEventListener('change', ()=> {
    if(pulsaLocked){
      // if locked, recompute preview (keep locked amount)
      computePulsaPreview();
    } else {
      hide(resultCard); hide(confirmWrap); hide(pulsaTargetWrap);
    }
  ...
  function computeEwalletPreview(){
    ...
    resultCard.classList.remove('hidden');
    // global WA confirm shown by caller
  }
  ...
  // typing on ewallet => hide preview
  ...
  // -----------------------
  // Confirm global -> Swal summary -> WA open (same logic)
  // -----------------------
