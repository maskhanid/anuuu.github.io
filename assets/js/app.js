/* assets/js/app.js
   Versi final (diperbarui):
   - Hanya tampilkan alamat network jika exchange + nominal dipilih
   - Pulsa / Ewallet: hasil otomatis update saat input berubah setelah konfirmasi
   - Mempertahankan Swal, history, WA pesan, onchain addresses
*/
...
(function(){
  ...
  // Confirm pulsa button still shows toast but uses same compute function
  ...
  // auto-update when user edits pulsa amount after confirmation
  pulsaAmountEl.addEventListener('input', ()=> {
    // If field non-empty and operator chosen -> recompute preview
  ...
  // auto-update when user edits ewallet amount after confirmation
