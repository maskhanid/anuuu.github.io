/* assets/js/app.js
   Versi lengkap final — menampung logika UI + konfirmasi WA + history + onchain addresses lengkap
   Pastikan index.html memanggil file ini dengan defer.
*/
...
(function(){
  ...
  // onchain addresses mapping (dari input user)
  const onchainAddrs = {
    // EVM-like networks (share same BEP20/ERC20 address)
  ...
  // -----------------------
  // Fee functions (same as original)
  // -----------------------
  ...
  function clearAll(){
    ...
    hide(confirmWrap); confirmWrap.classList.remove('slide-up');
  }
  ...
  function showMode(mode){
    ...
    // load saved prefs for mode-specific fields
    if(mode === 'crypto'){
    ...
    if(mode === 'pulsa'){
      const lastOp = loadPref('pulsaOp'); if(lastOp) pulsaOperator.value = lastOp;
      const lastAmt = loadPref('pulsaAmt'); if(lastAmt) pulsaAmountEl.value = String(lastAmt);
    }
    if(mode === 'ewallet'){
      const lastAmt = loadPref('ewalletAmt'); if(lastAmt) ewalletAmountEl.value = String(lastAmt);
    }
  }
  ...
  resetPrefs.addEventListener('click', ()=>{
    ...
    if(typeof Swal !== 'undefined') Swal.fire({icon:'success',title:'Preferensi direset',timer:900,showConfirmButton:false});
    setTimeout(()=> location.reload(), 700);
  });

  // -----------------------
  // Crypto compute & view (mirip original)
  // -----------------------
  ...
  async function updateCryptoView(){
    ...
    if(usdVal === 'custom' || usdVal === ''){ hide(resultCard); hide(confirmWrap); return; }
    usdVal = Number(usdVal);
    savePref('usd', usdVal);
    const chain = chainSelect.value || '';

    // avoid recompute if same
    ...
    // info area: show address if onchain selected
    infoArea.innerHTML = '';
  ...
  function renderChainIcon(chain){
    if(chain === 'bsc' || chain === 'bep20' || chain === 'avaevm' || chain === 'avaxc' || chain === 'eth' || chain === 'matic' || chain === 'arbitrum' || chain === 'optimism' || chain === 'opbnb' || chain === 'celo' || chain === 'kaia' || chain === 'plasma') {
  ...
  // events binding for crypto inputs
  ...
  document.body.addEventListener('click', (e)=>{
    ...
    if(addr){
      navigator.clipboard.writeText(addr).then(()=> {
        ...
        if(typeof Swal !== 'undefined') Swal.fire({icon:'info',title:'Salin',text:addr});
        else alert('Salin: ' + addr);
      });
    }
  });
  ...
  confirmEwallet.addEventListener('click', ()=>{
    ...
    // sample QR image (keep or replace with real)
  ...
  // -----------------------
  // Confirm -> Swal summary -> WA open (logic restored from original)
  // -----------------------
  confirmBtn.addEventListener('click', async ()=>{
    ...
    if(mode === 'crypto'){
      ...
      if(exIsOnchain){
        const chain = chainSelect.value || '';
        if(chain){ addrText = onchainAddrs[chain] || ''; summaryHtml += `Network: ${escapeHtml(chain.toUpperCase())}<br/>`; }
      ...
      const op = pulsaOperator.value || '-';
      const denom = toNumberFromInput(pulsaAmountEl.value) || 0;
    ...
    // build WA message (original format used %0A)
  ...
  clearHistoryBtn.addEventListener('click', ()=> {
    Swal.fire({
      ...
      if(res.isConfirmed){ localStorage.removeItem('mk_history'); renderHistory(); Swal.fire({icon:'success',title:'Riwayat dihapus',timer:900,showConfirmButton:false}); }
    });
  });
