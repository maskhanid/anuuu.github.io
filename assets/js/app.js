/* assets/js/app.js
   Versi final — perbaikan:
   - Crypto: segera tampilkan hasil dengan cached rate / BASE_RATE, kemudian refresh setelah fetchLiveRate()
   - Pulsa & E-Wallet: tetap lock on confirm (Konfirmasi -> tampilkan hasil/tujuan & Confirm WA; tombol berubah jadi Batal)
   - Lainnya: Swal, history, copy, onchain addresses preserved
*/

(function(){
  const BASE_RATE = 16500;
  const MAX_USD = 200;
  const waAdmin = '6281234567890'; // ganti sesuai admin WA
  ...
  // fill USD options (no default)
  (function populateUsd(){
    usdSelect.innerHTML = '<option value="">Pilih nominal (contoh: 1, 5, 10 ...)</option>';
    for(let i=1;i<=MAX_USD;i++){ const o=document.createElement('option'); o.value=i; o.textContent=i+' USD'; usdSelect.appendChild(o); }
  ...
  function savePref(k,v){ try{ localStorage.setItem('mk_pref_'+k, JSON.stringify(v)); }catch(e){} }
  function loadPref(k,def=null){ try{ const r=localStorage.getItem('mk_pref_'+k); return r?JSON.parse(r):def;}catch(e){return def;} }
  ...
  // fetch live rate (async). returns number
  async function fetchLiveRate(){
    try{
      const cached = loadRateCache();
      if(cached) {
        // still fetch in background to refresh cache, but return cached immediately
        fetch('https://api.exchangerate.host/latest?base=USD&symbols=IDR')
          .then(r=>r.ok? r.json():null)
          .then(j=>{ if(j && j.rates && j.rates.IDR){ const v=Math.round(j.rates.IDR); saveRateCache(v); } })
          .catch(()=>{});
        return cached;
      }
      ...
      console.warn('fetch rate failed, fallback to base', e);
      return BASE_RATE;
    }
  }

  // immediateFetchRate: use cached or BASE immediately; also start background fetch and update via callback
  function immediateFetchRate(callbackAfterFetch){
    const cached = loadRateCache();
    const immediate = (cached || BASE_RATE);
    // kick off network update asynchronously
    fetch('https://api.exchangerate.host/latest?base=USD&symbols=IDR')
      .then(r=> r.ok ? r.json() : null)
      .then(j => {
        if(j && j.rates && j.rates.IDR){
          const v = Math.round(j.rates.IDR);
          saveRateCache(v);
          if(typeof callbackAfterFetch === 'function') callbackAfterFetch(v);
        }
      }).catch(()=>{});
    return immediate;
  }
  ...
  // small sound
  function playErrorSound(){ try{ const ctx = new (window.AudioContext || window.webkitAudioContext)(); const o=ctx.createOscillator(), g=ctx.createGain(); o.type='sine'; o.frequency.value=220; g.gain.value=0.0001; o.connect(g); g.connect(ctx.destination); o.start(); g.gain.exponentialRampToValueAtTime(0.08, ctx.currentTime+0.02); g.gain.exponentialRampToValueAtTime(0.0001, ctx.currentTime+0.16); setTimeout(()=>{ try{ o.stop(); ctx.close(); }catch(e){} },300); }catch(e){} }
  ...
  // Crypto compute: now uses immediate cached rate then refreshes when live arrives
  let lastComputed = { mode:'', ex:'', usd:0, chain:'' };
  async function updateCryptoView(doImmediate=true){
    ...
    if(!usdVal || usdVal === 'custom'){
      // hide everything until nominal chosen
      infoArea.innerHTML = '';
      hide(infoArea);
      hide(resultCard);
      hide(confirmWrap);
    ...
    // immediate calculation using cached rate or BASE
    const immediateRate = immediateFetchRate((freshRate) => {
      // callback: refresh UI if rate changed or recompute necessary
      // only update if the same parameters still active
      if(lastComputed.mode === modeSelect.value && lastComputed.ex === ex && lastComputed.usd === usdVal && lastComputed.chain === chain){
        applyCryptoCalculation(usdVal, freshRate, ex, chain); // refresh UI
      }
    });

    // apply immediate result
    applyCryptoCalculation(usdVal, immediateRate, ex, chain);
  ...
  function applyCryptoCalculation(usdVal, rate, ex, chain){
    let rup, fee;
    ...
    // show address only when exchange chosen and nominal chosen (and chain if onchain)
    if(ex === 'Wallet Crypto / Onchain'){
      if(!chain){
        ...
        `;
        infoArea.classList.remove('hidden');
        // enable confirm only when exchange chosen and (if onchain) chain chosen
        const ready = (ex && (ex !== 'Wallet Crypto / Onchain' || chain));
        if(ready){ confirmWrap.classList.remove('hidden'); confirmWrap.classList.add('slide-up'); }
        else hide(confirmWrap);
      }
    } else {
      ...
      infoArea.classList.remove('hidden');
      const ready = (ex);
      if(ready){ confirmWrap.classList.remove('hidden'); confirmWrap.classList.add('slide-up'); } else hide(confirmWrap);
    }
  }
  ...
  cryptoExchange.addEventListener('change', ()=> {
    // if nominal not chosen, clear info & hide
    if(!usdSelect.value || usdSelect.value === 'custom'){ infoArea.innerHTML=''; hide(infoArea); hide(resultCard); hide(confirmWrap); }
    updateCryptoView();
  });
  usdSelect.addEventListener('change', ()=> {
    if(!usdSelect.value || usdSelect.value === 'custom'){ infoArea.innerHTML=''; hide(infoArea); hide(resultCard); hide(confirmWrap); return; }
    updateCryptoView();
  });
  chainSelect.addEventListener('change', ()=> updateCryptoView());
  ...
  pulsaOperator.addEventListener('change', ()=> {
    if(pulsaLocked) computePulsaPreview(); else setTimeout(()=> { if(pulsaAmountEl.value) computePulsaPreview(); }, 80);
    savePref('pulsaOp', pulsaOperator.value);
  });

  // E-Wallet: compute & lock toggle
  function computeEwalletPreview(){
    ...
    const fee = ewalletFee(num), received = num - fee;
    resultMain.textContent = "Rp " + formatIDR(received);
  ...
  ewalletAmountEl.addEventListener('input', ()=> {
    ...
    if(ewalletAmountEl.value) computeEwalletPreview(); else { hide(resultCard); hide(confirmWrap); hide(ewalletResultImg); }
  });

  // Confirm WA (summary modal + open wa)
  ...
  (function init(){
    ...
    // prefetch live rate to warm cache
    fetchLiveRate().catch(()=>{});
    renderHistory();
  })();
