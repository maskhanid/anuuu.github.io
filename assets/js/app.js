/* assets/js/app.js
   Versi: crypto-infoarea-hide-until-exchange + copy-icon-inline
   Memperbaiki:
   - Sembunyikan infoArea jika exchange belum dipilih (meskipun nominal sudah dipilih)
   - Untuk onchain: infoArea hanya tampil jika network dipilih
   - Tombol copy untuk exchange tampil inline di sebelah kanan ID
   - Fitur lain tetap: cached rate, pulsa/ewallet confirm lock, history, WA, dll.
*/

(function(){
  const BASE_RATE = 16500;
  const MAX_USD = 200;
  const waAdmin = '6281234567890'; // ganti jika perlu
  ...
  // populate USD options (no default)
  ...
  // immediate fetch (cached immediate + background refresh)
  ...
  // -----------------------
  // Crypto compute & view (result shown even if exchange not chosen)
  // infoArea only shown when exchange selected (and for onchain when network is selected)
  // -----------------------
  ...
  async function updateCryptoView(doImmediate=true){
    ...
    const immediateRate = immediateFetchRate((freshRate) => {
      if(lastComputed.mode === modeSelect.value && lastComputed.ex === ex && lastComputed.usd === usdVal && lastComputed.chain === chain){
        applyCryptoCalculation(usdVal, freshRate, ex, chain);
      }
    });
  ...
  function applyCryptoCalculation(usdVal, rate, ex, chain){
    // compute and always render result card
    let rup, fee;
    ...
    // show infoArea only when exchange is selected
    if(!ex){
      // exchange not chosen -> hide info area and disable confirm
      infoArea.innerHTML = '';
      hide(infoArea);
      hide(confirmWrap);
      return;
    ...
    // exchange chosen -> render info area
    if(ex === 'Wallet Crypto / Onchain'){
      // onchain: only show when network selected
      if(!chain){
        infoArea.innerHTML = '';
        hide(infoArea);
        hide(confirmWrap);
        ...
        // inline layout: left (label + addr) right (copy button)
        infoArea.innerHTML = `
          <div class="addr-box" style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
            <div style="flex:1">
              <div class="text-xs muted flex items-center gap-2">${icon}<span class="font-semibold">${label}</span></div>
              <div class="addr-text mt-2">${addr}</div>
            </div>
            <div style="margin-left:12px;">
              <button class="copy-icon" data-copy-addr aria-label="Salin alamat" title="Salin alamat">
                <svg viewBox="0 0 24 24" fill="none" width="22" height="22"><path d="M16 4H8a2 2 0 0 0-2 2v10" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/><rect x="9" y="8" width="10" height="12" rx="2" stroke="currentColor" stroke-width="1.6"/></svg>
              </button>
            </div>
          </div>
      ...
      // centralized exchange: render id with copy inline on right
      ...
      infoArea.innerHTML = `
        <div class="addr-box" style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
          ...
          <div style="margin-left:12px;">
            <button class="copy-icon" id="copyExId" aria-label="Salin ID Exchange" title="Salin ID">
              <svg viewBox="0 0 24 24" fill="none" width="22" height="22"><path d="M16 4H8a2 2 0 0 0-2 2v10" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/><rect x="9" y="8" width="10" height="12" rx="2" stroke="currentColor" stroke-width="1.6"/></svg>
            </button>
          </div>
        </div>
  ...
  cryptoExchange.addEventListener('change', ()=> {
    // When exchange changes: if nominal chosen, show result but hide infoArea until ready
  ...
  // delegated copy handler (works for both inline copy buttons)
  ...
  // -----------------------
  // Pulsa: show preview only after Konfirmasi (unchanged from previous)
  // -----------------------
  ...
  confirmPulsa.addEventListener('click', ()=>{
    if(!pulsaLocked){
      ...
      confirmWrap.classList.remove('hidden'); confirmWrap.classList.add('slide-up');
      if(typeof Swal !== 'undefined') Swal.fire({toast:true,position:'top-end',icon:'success',title:'Nominal terkonfirmasi',showConfirmButton:false,timer:900});
    } else {
      pulsaLocked = false;
  ...
  pulsaOperator.addEventListener('change', ()=> {
    if(pulsaLocked) computePulsaPreview(); else { hide(resultCard); hide(confirmWrap); hide(pulsaTargetWrap); }
    savePref('pulsaOp', pulsaOperator.value);
  });

  // -----------------------
  // E-Wallet: same behavior as pulsa
  // -----------------------
  ...
  // -----------------------
  // Confirm global -> Swal summary -> WA open
  // -----------------------
